...
- `join_keys.md` — how tables connect: 152 join columns, plus bridges for sources that name the same key differently (`UF`, `codIBGE`, `MUNIC`, `CPF_CNPJ`…) with a tested normalization expression each. Generated by `scripts/gera_join_keys.py` — regenerate, never hand-edit
...
| `rows`, `num_files`, `size_bytes` | medidos via `parquet_metadata`, incluindo tabelas particionadas |
...
| `source` | `disk` (parquet local) ou `view_only` (view sem parquet — dado sumiu; sempre `rows=0`) |
| `provenance_notes` | notas do `datasets_to_scrap.md`, truncadas em 500 chars |

Ao contar tabelas ou linhas, filtre `source='disk'` — as 8 linhas `view_only` são views órfãs mantidas de propósito para a quebra ficar visível.
...
- Quatro distribuições, todas pré-calculadas em Python e embarcadas no JSON: `grade` (cartões empacotados), `gravidade` (n corpos), `orbita` (anéis por chave), `temas` (um território por tema).
