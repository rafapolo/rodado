...
import { legenda } from "./dicionarios.ts";
 ...
 * de três.
 */
export function descreve(tabela: string, cols: Coluna[], sufixo = "", filtro = ""): string {
  ...
  const linha = (c: Coluna) => {
    ...
    const v = legenda(tabela, c.name) ? undefined : valores(tabela, c.name);
    const vs = v ? ` — valores${v.amostra ? " (amostra)" : ""}: ${v.lista.map((x) => `'${x}'`).join(", ")}` : "";
    return `  ${c.name}: ${curto(c.type)}${legenda(tabela, c.name)}${vs}${n ? ` — NOTA: ${n}` : ""}`;
  };
  ...
  if (termos.length) {
    ...
    return [cab, ...destaque.map(linha), `  (+${achadas.length} com '${termos.join("', '")}')`, ...achadas.map(linha)].join("\n");
  }
  if (cols.length <= LARGA) return [cab, ...cols.map(linha)].join("\n");
