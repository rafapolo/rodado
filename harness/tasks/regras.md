...
| 2+ `LIKE` encadeados por OR contra a mesma coluna de CID é rejeitado | mesmo erro do `BETWEEN`, outra porta: benchmark pós-`agente.ts` (2026-09-04) pediu suicídios RJ 2020 (X60-X84, esperado 789) e voltou **128** — `causa_basica LIKE 'X60%' OR ... OR 'X69%'`, dez cláusulas bem formadas que esqueceram X70-X84 inteiro. Não valida se a faixa está completa (não dá, sem saber o que a pergunta pediu) — rejeita a FORMA e ensina `substr(...) BETWEEN` | `portao.ts` camada 6, `portao.test.ts` |
...
| Cache de prefixo entre SESSÕES separadas nem sempre pega, mesmo com system+tools byte-idênticos | medido 2026-09-03, rodada real do item 2 de `backlog.md` (laço agêntico completo, não `avalia_datasets.ts`): dentro de uma sessão o cache funciona perfeito (cada turno paga só o incremento genuíno); ao trocar de PERGUNTA — sessão nova — o slot único (`-np 1`) reprocessa o prefixo inteiro (~7.100–7.200 tokens) em vez dos ~5–45 tokens do caso isolado que sustentava o "44x". Confirmado por log do `llama-server`: o `system`+`tools` das duas sessões são **byte-idênticos** (JSON comparado direto), então não é o "nada variável entra no prefixo" de cima — é limite do jeito como o `llama-server` reaproveita KV entre sequências que DIVERGEM logo após o prefixo comum (a conversa anterior é longa e específica do caso; a nova é curta), não só entre sequências que uma é extensão da outra | 🔴 aberto — não derruba o total por caso na prática (o tempo observado por caso, 170–303 s, ainda bate com a estimativa de ~6 min), mas invalida a suposição de que a rodada inteira herda o 44x depois do primeiro caso; a estimativa de tempo de qualquer rodada multi-pergunta pelo laço agêntico deveria contar com isso, não com o "quase grátis" medido em `avalia_datasets.ts` (que chama o modelo direto, sem sessão nova por caso) |
...
| O modelo **não pode ter shell** | descobriu a ferramenta `bash` e consultou o DuckDB por fora do portão — validação inteira vira decoração | ✅ travado por construção — o laço (`agente.ts`) só monta as ferramentas que `mcp.ts` expõe, `bash` nunca existe ([`operacao.md`](operacao.md) tarefa 4) |
...
| O modelo às vezes para num "plano" pedindo aprovação, em vez de executar | achado 2x em rodadas diferentes 2026-09-03, diagnosticando o item 10: o turno termina com "aguardando sua aprovação para executar as consultas SQL" — comportamento herdado de contexto assistente-interativo que não se aplica aqui (não há humano pra aprovar). Mesma família do "erro de não iterar" acima, agora medida no laço agêntico, não só no pipeline aposentado | ✅ `PERSONA` (`agente.ts`) instrui explicitamente a nunca parar num plano — testado ao vivo, a mesma pergunta que produziu o plano passou a executar e concluir |
| Tool call do Gemma nem sempre é reconhecido pelo parser do `llama-server` | medido 2026-09-03, rodada real do item 2 de `backlog.md` (a primeira vez que o laço encadeou muitas sessões reais de verdade): 4 de 6 sessões terminaram com a chamada de ferramenta em formato NATIVO do Gemma (`<\|tool_call>call:nome{...}<tool_call\|>`, tags na ordem trocada) caindo como bloco de `reasoning` — texto solto, não executado — em vez de tool call reconhecido. Não é erro de raciocínio: o conteúdo (inclusive SQL) costuma estar correto | 🔴 aberto, bloqueador — item 10 do [`backlog.md`](backlog.md). `--no-jinja` testado e descartado (quebra o servidor inteiro pra este checkpoint); `--chat-template`/`--grammar` explícitos ainda não tentados |
...
que restam nas tabelas de cima (cache entre sessões separadas, tool call do
...
ao vivo (rodar o caso "573 em vez de 789" pelo laço agêntico de novo e confirmar que o
