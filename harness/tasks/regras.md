...
| Nome semântico distingue domínio, **não distingue irmão** | no conjunto inteiro, **24 das 36 falhas** são o parente errado: `ibge_ppm`↔`ibge_pam`, `anp_combustiveis`↔`anp_precos_combustiveis`, `me_caged`↔`me_rais` | 🟡 codado — `desambigua.ts` + `dados/desambiguacao.json`, wireado em `prefixo.ts` e (depois de um bug medido: media o catálogo sem a pista) `avalia_datasets.ts` — item 1 do [`backlog.md`](backlog.md), medição ao vivo em andamento |
