...
| [`fontes_novas.md`](fontes_novas.md) | O que existe de dado público **fora** do espelho, rankeado por valor/esforço | 🔵 backlog — Gás do Povo (20.817.231 linhas), Novo Bolsa Família (821.346.847, 41/41 meses) e Transferegov/SICONV (66.107.314 linhas, 48 tabelas, 5 zips multi-membro pendentes) terminaram sozinhos no beelink sem supervisão, todos conferidos ao vivo em 2026-09-02 — **nenhum dos três tem view no `.duckdb` ainda nem passou pelo regen de metadados**, próximo passo documentado no arquivo; PNCP subiu de 1.412.466 para 1.979.024 linhas e parou sozinho ("fonte esgotada", rate limit) em vez de terminar; vários itens do resto do backlog seguem presos em credencial/API |
...
|---|---|---|
| **PNCP mai/2025 → hoje** | **1.979.024 contratos** em `br_pncp.contratos` (2021–2026, conferido 2026-09-02, subiu de 1.412.466) — o job rodou sem supervisão e parou sozinho: `persistente.log` no beelink registra "sem progresso entre rodadas — fonte esgotada, parando" às 18:49 de 2026-09-02, depois de 3 rodadas travadas em ~3.545 páginas faltando | O `HTTP 500` da API é **rate limit por IP** — confirmado por um `429` explícito. Paralelizar do mesmo IP não adianta (todos dividem a cota); o proxy BR tem cota própria e devolve 200 onde o direto dá 500. `_staging/pncp/duplo.sh` roda as duas rotas, ~20h. Retomar: `nohup bash ~/rodado/_staging/pncp/duplo.sh &` — ele pula por `stat` o que já existe, mas vale esperar a cota de IP abrir de novo antes ("fonte esgotada" é cota batida, não bug) |
