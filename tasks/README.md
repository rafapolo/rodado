...
| [`plan/ux_analises.md`](plan/ux_analises.md) | UX de `pages/analises/`: índice escaneável (dek com `line-clamp`, tag virando filtro, lista também renderizada estática — hoje `/analises/` sem JS é só "Carregando…") e artigo navegável (sumário sticky no gutter de ~450px que já sobra, tempo de leitura, `.pager` reusado dos temas). 🔴 nada implementado — 6 itens, nenhum com dependência nova. Medido: 13 itens no manifest com dek de 269 a 779 chars, textos de 1.522 a 4.328 palavras com 3 a 14 seções. Inclui uma decisão de conteúdo pendente: `mapa-da-saude-mental` tem `.md`, pasta, shell e og-image mas está fora do `manifest.json` — entra ou sai |
...
| [`plan/fix_cpf_lookup_encoding.md`](plan/fix_cpf_lookup_encoding.md) | `main.cpf_lookup` (223.670.206 linhas de nome+CPF, não documentada em nenhum catálogo) tem conteúdo corrompido que quebra `information_schema.columns`/`duckdb_columns()` pro banco inteiro — achado ao corrigir `br_mjsp_ckan.infopen` (esse já corrigido, ver `sync_mcp_schema.py`). 🔴 só diagnosticado, nada corrigido de propósito — é PII em escala, tema irmão da decisão de privacidade ainda aberta em `done/deanonimizacao_geral.md`; decidir como prosseguir fica para quem já tem esse contexto |
| [`plan/avaliacao_infra_pesada.md`](plan/avaliacao_infra_pesada.md) | Iceberg/Airflow/NiFi/CI-CD/Kafka avaliados contra a arquitetura real (mirror local, sem serviço live, sem escritor concorrente) — pedido em conversa. 🔴 nada implementado; único item com ação concreta é CI/CD (`validate.yml` rodando `valida_metrics.py` em PR que toque `docs/context/*.yaml` — confirmado que não precisa de acesso a beelink) |
