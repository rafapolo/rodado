...
- `schema_dict_status.json` — estágios 1+2 de [`tasks/generate-full-schema-dict.md`](tasks/generate-full-schema-dict.md), mais uma passada de leitura humana/LLM (não regex) por cima: toda coluna STRING/INTEGER fora de `dicionario_coverage.json` etiquetada (`dicionario_disponivel` fica implícito, fora do escopo do arquivo) — 28.263 colunas: **8.536 `nao_verificado`** (candidato real a código, sem fonte de significado em lugar nenhum — a etiqueta que importa, generaliza o alerta manual que `tasks/harness/backlog.md` item 9 teve que escrever à mão pra `br_ms_sim.circunstancia_obito`), 15.872 `nao_e_codigo` (nome autoexplicativo, cardinalidade alta demais, ou — a maior fatia — confirmado por leitura direta: SISDEPEN inteiro, 3.233 colunas, é a pergunta literal do formulário oficial convertida em slug), 2.549 `documentado_em_outro_lugar` (inclui os 2.228 códigos V- do "Agregados por Setores Censitários" do Censo 2010, achado com dicionário oficial do IBGE mas não conferido célula a célula), 1.306 `padrao_externo` (CNAE/CID/CBO/NCM/geografia, já cobertos por `hierarchies.yaml`/`bridges.yaml`). Números de 2026-09-25, depois do estágio 4. Tabelas acima de 50M linhas (6,3 bilhões no maior caso) têm a cardinalidade adiada por custo, não medida. Gerado por `scripts/gera_schema_dict_status.py` + `scripts/llm_triage_schema_dict_status.py` + `scripts/aplica_column_codes.py`; `describe_table` lê e expõe `nao_verificado_warning` por tabela
- `column_codes.yaml` — **escrito à mão**: o significado pesquisado (fonte oficial + valores medidos no beelink) de 186 colunas-código que nada decodificava — SINAN violência/malária/esquistossomose, SIM, Siconfi `estagio_bd`, MapBiomas, natureza jurídica, `id_municipio_*`. `describe_table` devolve no bloco `column_codes`; `aplica_column_codes.py` propaga o status para `schema_dict_status.json`. A fila do que pesquisar a seguir é `schema_dict_prioridade.json` (`scripts/prioriza_schema_dict_status.py`, ranking por uso real)
