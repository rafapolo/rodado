#!/usr/bin/env python3
"""
Fetch SINAN Esquistossomose (esquistossomose mansônica) microdata from
DataSUS's public FTP -> Parquet -> beelink.

`br_ms_sinan` on Base dos Dados only mirrors dengue and influenza/SRAG --
esquistossomose was never on Base dos Dados at all. Same pipeline shape as
...
listing the FTP directories directly (2026-09-01):
  - ftp://ftp.datasus.gov.br/dissemin/publicos/SINAN/DADOS/FINAIS/ESQUBR{YY}.dbc
    (consolidated, 2007-2022)
  - ftp://ftp.datasus.gov.br/dissemin/publicos/SINAN/DADOS/PRELIM/ESQUBR{YY}.dbc
    (preliminary, 2023-2026)
...
Usage:
    python3 scripts/scrap/sinan_esquistossomose.py
"""
...
BEELINK_PATH = "~/rodado/br_ms_sinan_esquistossomose/microdados_esquistossomose"
FTP_BASE = "ftp://ftp.datasus.gov.br/dissemin/publicos/SINAN/DADOS"
TEMP_DIR = Path(
    "/private/tmp/claude-501/-Users-polux-Projetos-rodado/0c20fa88-739d-4371-b790-94956f16aa3f/scratchpad/sinan_esquistossomose"
)
...
# 2026-09-01: FINAIS covers 2007-2022, PRELIM covers 2023-2026.
YEARS = [(2000 + yy, f"{yy:02d}", "FINAIS") for yy in range(7, 23)] + [
    (2000 + yy, f"{yy:02d}", "PRELIM") for yy in range(23, 27)
]


def fetch_dbc(year: int, suffix: str, subdir: str) -> Path:
    url = f"{FTP_BASE}/{subdir}/ESQUBR{suffix}.dbc"
    dest = TEMP_DIR / f"ESQUBR{suffix}.dbc"
...
def push(new_records: list, label: str, existing_path: Path = None) -> bool:
    ...
    parquet_path = TEMP_DIR / "microdados_esquistossomose.parquet"
    con = duckdb.connect()
...
def already_done_years() -> tuple:
    ...
    result = subprocess.run(
        f"rsync -a {BEELINK_HOST}:{BEELINK_PATH}/microdados_esquistossomose.parquet {baseline_path}",
