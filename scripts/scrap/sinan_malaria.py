#!/usr/bin/env python3
"""
Fetch SINAN Malaria microdata from DataSUS's public FTP -> Parquet -> beelink.

IMPORTANT distinction: Brazil runs two separate malaria surveillance systems.
SIVEP-Malaria is the case registry for the Amazonia Legal region (where the
disease is endemic and case volume is high). SINAN's MALA group is where
malaria cases occurring OUTSIDE Amazonia Legal are notified -- exactly the
relevant system for a non-endemic municipality like Nova Friburgo/RJ. This
script fetches the SINAN (non-endemic-area) series, not SIVEP-Malaria.

`br_ms_sinan` on Base dos Dados only mirrors dengue and influenza/SRAG --
malaria was never on Base dos Dados at all. Same pipeline shape as
...
listing the FTP directories directly (2026-09-01):
  - ftp://ftp.datasus.gov.br/dissemin/publicos/SINAN/DADOS/FINAIS/MALABR{YY}.dbc
    (consolidated, 2004-2022)
  - ftp://ftp.datasus.gov.br/dissemin/publicos/SINAN/DADOS/PRELIM/MALABR{YY}.dbc
...
Usage:
    python3 scripts/scrap/sinan_malaria.py
"""
...
BEELINK_PATH = "~/rodado/br_ms_sinan_malaria/microdados_malaria"
FTP_BASE = "ftp://ftp.datasus.gov.br/dissemin/publicos/SINAN/DADOS"
TEMP_DIR = Path(
    "/private/tmp/claude-501/-Users-polux-Projetos-rodado/0c20fa88-739d-4371-b790-94956f16aa3f/scratchpad/sinan_malaria"
)
...
YEARS = [(2000 + yy, f"{yy:02d}", "FINAIS") for yy in range(4, 23)] + [
    (2000 + yy, f"{yy:02d}", "PRELIM") for yy in range(23, 25)
]


def fetch_dbc(year: int, suffix: str, subdir: str) -> Path:
    url = f"{FTP_BASE}/{subdir}/MALABR{suffix}.dbc"
    dest = TEMP_DIR / f"MALABR{suffix}.dbc"
...
def push(new_records: list, label: str, existing_path: Path = None) -> bool:
    ...
    parquet_path = TEMP_DIR / "microdados_malaria.parquet"
    con = duckdb.connect()
...
def already_done_years() -> tuple:
    ...
    result = subprocess.run(
        f"rsync -a {BEELINK_HOST}:{BEELINK_PATH}/microdados_malaria.parquet {baseline_path}",
