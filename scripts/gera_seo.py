...
def alvos_head() -> list[Path]:
    ...
    Exceptions: `atlas/` (generated by `build_atlas.py`, its own fonts and
    CSS — see CLAUDE.md) and `api/` (embeds Redoc, which renders into the
    global DOM with no Shadow DOM). Both are self-contained by design;
    injecting the shared block here swaps out the font/CSS the page chose
    for itself and — in Redoc's case — leaks `p { margin: 0 0 1.25rem }` and
    site.css's `--bg` into the widget, breaking its layout (seen live:
    2026-08-27, fixed after it had already been injected into both once by
    mistake).
    """
    EXCLUI = {"atlas", "api"}
    return sorted(
