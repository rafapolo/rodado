...
Entity/relationship map of the 825 tables (195 datasets) in the mirror. Generated by `scripts/gera_erd.py` from `schemas.json` on 2026-07-27 — do not edit by hand, regenerate.

📊 **PDF posters** (generated by `scripts/gera_erd_poster.py`):
  - [`ERD-poster-domain.pdf`](ERD-poster-domain.pdf) — aggregated view: hubs + domains (A1, ~62 KB)
  - [`ERD-poster-full.pdf`](ERD-poster-full.pdf) — full catalogue: all 195 datasets listed (A0, ~47 KB)
...
One `erDiagram` with 825 tables would be unreadable, so the model sits one level up:
...
| Politics and elections | 6 | 60 | 6 |
| Justice, security and sanctions | 21 | 52 | 12 |
| Territory, environment and infrastructure | 21 | 80 | 18 |
| Demographics and social indicators | 17 | 123 | 15 |
| International, culture and sport | 9 | 25 | 3 |
| **total** | **195** | **825** | **158** |

37 datasets have no documented key at all; 201 individual tables carry no key (both listed at the end).
...
erDiagram
    ...
    br_bd_diretorios_brasil {
        no_key area_conhecimento "1.8k rows"
        cbo cbo_1994 "4.8k rows"
        cbo cbo_2002 "5.6k rows"
        mun_uf_cep cep "905.2k rows"
        cid cid_10 "25k rows"
        cid cid_9 "1.8k rows"
        cnae cnae_1 "1.2k rows"
        cnae cnae_2 "2.7k rows"
        no_key curso_superior "510 rows"
        mun_uf distrito_1991 "17.7k rows"
        mun_uf distrito_2000 "19.7k rows"
        mun_uf distrito_2010 "20.6k rows"
        mun_uf_cep_cnpj_cnae empresa "empty"
        mun_uf_escola escola "436.2k rows"
        no_key etnia_indigena "528 rows"
        mun_uf_ies instituicao_ensino_superior "13.2k rows"
        mun_uf municipio "11.1k rows"
        no_key natureza_juridica "204 rows"
        uf regiao "10 rows"
        mun_uf_setor setor_censitario_2010 "620.2k rows"
        mun_uf_setor setor_censitario_2022 "904.7k rows"
        no_key subatividade_ibge "572 rows"
        uf uf "54 rows"
    }
    ...
    br_bd_diretorios_mundo {
        uf continente "14 rows"
        ncm nomenclatura_comum_mercosul "27.5k rows"
        pais pais "526 rows"
        ncm sistema_harmonizado "13.2k rows"
    }
    br_bd_diretorios_us {
...
erDiagram
    ...
    br_bcb_estban {
        ...
        no_key dicionario "228 rows"
        mun_uf_cnpj_ano_mes municipio "256.2M rows"
    }
    br_bcb_sgs {
        no_key series "24.9k rows"
    }
    br_bcb_sicor {
        no_key dicionario "816 rows"
        no_key empreendimento "6.6k rows"
        ano_mes liberacao "21M rows"
        uf_cnpj_funcprog_ano_mes operacao "27.2M rows"
        ano_mes operacoes_desclassificadas "12.7k rows"
        mun_cnpj_ano_mes recurso_publico_complemento_operacao "22M rows"
        cnpj_cpf_funcprog_ano_mes recurso_publico_cooperado "228.3k rows"
...
erDiagram
    ...
    br_me_rais {
        ...
        mun_uf_cep_cnae_ano microdados_estabelecimentos "231.8M rows"
...
erDiagram
    ...
    br_camara_dados_abertos {
        mun_uf deputado "15.8k rows"
        mun_uf_cpf_partido deputado_contato "7.6k rows"
        uf_ano deputado_ocupacao "74.8k rows"
        no_key deputado_profissao "25.1k rows"
        uf_cnpj_cpf_partido_ano_mes despesa "4.8M rows"
        no_key evento "121k rows"
        no_key evento_orgao "124.1k rows"
        no_key evento_presenca_deputado "2.7M rows"
        no_key evento_requerimento "42.3k rows"
        no_key frente "2.9k rows"
        no_key frente_deputado "523.7k rows"
        no_key funcionario "26.8k rows"
        ano legislatura "114 rows"
        uf_partido legislatura_mesa "310 rows"
        ano licitacao "9.3k rows"
        uf_cnpj_cpf_ano licitacao_contrato "2.7k rows"
        cnpj_cpf_ano licitacao_item "127.1k rows"
        ano licitacao_pedido "5.6k rows"
        cnpj_cpf_ano licitacao_proposta "83.3k rows"
        uf orgao "4.7k rows"
        uf_partido orgao_deputado "183.7k rows"
        uf_partido proposicao_autor "3.8M rows"
        uf_ano proposicao_microdados "910.6k rows"
        ano proposicao_tema "639.9k rows"
        no_key sigla_partido "143 rows"
        no_key votacao "367.6k rows"
...
erDiagram
    ...
    br_ipea_atlasviolencia {
        no_key series "10 rows"
        ano valores_nacional "182 rows"
    }
...
erDiagram
    ...
    br_anatel_banda_larga_fixa {
        ano_mes densidade_brasil "384 rows"
        mun_uf_ano_mes densidade_municipio "2.1M rows"
        uf_ano_mes densidade_uf "10.4k rows"
        mun_uf_cnpj_ano_mes microdados "57.8M rows"
    }
    br_anatel_indice_brasileiro_conectividade {
        mun_uf_ano municipio "44.6k rows"
    }
    br_geobr_mapas {
...
erDiagram
    ...
    br_abrinq_oca {
        mun_ano municipio_primeira_infancia "111.4k rows"
    }
