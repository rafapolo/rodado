...
Entity/relationship map of the 834 tables (198 datasets) in the mirror. Generated by `scripts/gera_erd.py` from `schemas.json` on 2026-08-23 — do not edit by hand, regenerate.
...
One `erDiagram` with 834 tables would be unreadable, so the model sits one level up:
...
| Territory, environment and infrastructure | 23 | 87 | 19 |
| Demographics and social indicators | 17 | 123 | 15 |
| International, culture and sport | 9 | 25 | 3 |
| **total** | **198** | **834** | **160** |

38 datasets have no documented key at all; 204 individual tables carry no key (both listed at the end).
