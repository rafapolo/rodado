...
| Arquivo | O que é | Gerado por |
|---|---|---|
| `all_tables.txt` | 900 `dataset.tabela`, uma por linha — a lista chapada, incluindo as 8 tabelas nativas do `.duckdb` que não têm parquet | `build_metadata_catalog.py` |
| `basedosdados-schema.json` | Schema completo que o `describe_table` do MCP lê (211 datasets, 893 tabelas, 39.577 colunas) | `sync_mcp_schema.py`, a partir de `schemas.json` na raiz |
